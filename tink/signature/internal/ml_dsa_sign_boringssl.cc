...
absl::StatusOr<std::unique_ptr<PublicKeySign>> MlDsa44SignBoringSsl::New(
    const MlDsaPrivateKey& private_key, absl::string_view context) {
  absl::Status status =
      internal::CheckFipsCompatibility<MlDsa44SignBoringSsl>();
  if (!status.ok()) {
    return status;
  }
...
absl::StatusOr<std::string> MlDsa44SignBoringSsl::Sign(
...
absl::StatusOr<std::unique_ptr<PublicKeySign>> NewMlDsaSignWithContextBoringSsl(
  ...
  switch (private_key.GetPublicKey().GetParameters().GetInstance()) {
    case MlDsaParameters::Instance::kMlDsa44:
      return MlDsa44SignBoringSsl::New(std::move(private_key), context);
