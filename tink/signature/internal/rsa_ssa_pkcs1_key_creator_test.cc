...
TEST_P(KeyCreatorsTest, CreateRsaSsaPkcs1PrivateKeyWorks) {
  ...
  absl::StatusOr<std::unique_ptr<RsaSsaPkcs1PrivateKey>> private_key =
      CreateRsaSsaPkcs1Key(*parameters, test_case.id_requirement);
...
TEST_P(KeyCreatorsTest, CreateRsaSsaPkcs1PrivateKeyTwiceYieldsDifferentKeys) {
  TestCase test_case = GetParam();
  ...
  absl::StatusOr<std::unique_ptr<RsaSsaPkcs1PrivateKey>> private_key1 =
      CreateRsaSsaPkcs1Key(*parameters, test_case.id_requirement);
  ASSERT_THAT(private_key1, IsOk());

  absl::StatusOr<std::unique_ptr<RsaSsaPkcs1PrivateKey>> private_key2 =
      CreateRsaSsaPkcs1Key(*parameters, test_case.id_requirement);
...
TEST_P(KeyCreatorsTest, CreateKeysetHandleFromConfigWithRsaSsaPkcs1KeyWorks) {
  ...
  ASSERT_THAT(
      internal::KeyGenConfigurationImpl::AddKeyCreator<RsaSsaPkcs1Parameters>(
          CreateRsaSsaPkcs1Key, key_creator_config),
      IsOk());
