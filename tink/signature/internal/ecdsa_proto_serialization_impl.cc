...
absl::StatusOr<EcdsaPrivateKey> ParsePrivateKey(
  ...
  absl::StatusOr<SecretData> key_value = ParseBigIntToFixedLength(
      SecretDataAsStringView(proto_key.key_value()),
  ...
  return EcdsaPrivateKey::Create(*public_key, std::move(private_key_value),
                                 GetPartialKeyAccess());
}
