...
// Tests for LegacyKeyManagerImpl.
//
// These tests use the ML-DSA key manager as a representative example the
// conversion from key classes to the legacy KeyManager API.
class LegacyKeyManagerImplTest : public ::testing::Test {
 protected:
  void SetUp() override {
    ASSERT_THAT(SignatureConfig::Register(), IsOk());
    ml_dsa_sign_key_manager_ = MakeMlDsaSignKeyManager();
    ml_dsa_verify_key_manager_ = MakeMlDsaVerifyKeyManager();
  }

  std::unique_ptr<KeyManager<PublicKeySign>> ml_dsa_sign_key_manager_;
  std::unique_ptr<KeyManager<PublicKeyVerify>> ml_dsa_verify_key_manager_;
};

TEST_F(LegacyKeyManagerImplTest, Basics) {
  EXPECT_THAT(ml_dsa_sign_key_manager_->get_version(), Eq(0));
  EXPECT_THAT(ml_dsa_sign_key_manager_->get_key_type(),
              Eq("type.googleapis.com/google.crypto.tink.MlDsaPrivateKey"));
}

TEST_F(LegacyKeyManagerImplTest, Factory_NewKey) {
  const KeyFactory& ml_dsa_factory =
      ml_dsa_sign_key_manager_->get_key_factory();

  auto key_format = google::crypto::tink::MlDsaKeyFormat();
  key_format.set_version(0);
  key_format.mutable_params()->set_ml_dsa_instance(
      google::crypto::tink::MlDsaInstance::ML_DSA_65);

  absl::StatusOr<std::unique_ptr<portable_proto::MessageLite>> key =
      ml_dsa_factory.NewKey(key_format);
  ...
  EXPECT_THAT(ml_dsa_key->public_key().version(), Eq(0));
  EXPECT_THAT(ml_dsa_key->public_key().key_value().size(), Eq(1952));
  EXPECT_THAT(ml_dsa_key->public_key().version(), Eq(0));
  EXPECT_THAT(ml_dsa_key->public_key().params().ml_dsa_instance(),
              Eq(google::crypto::tink::MlDsaInstance::ML_DSA_65));
}

TEST_F(LegacyKeyManagerImplTest, Factory_NewKey_SerializedKeyFormat) {
  const KeyFactory& ml_dsa_factory =
      ml_dsa_sign_key_manager_->get_key_factory();
...
TEST_F(LegacyKeyManagerImplTest, Factory_NewKeyData) {
  const KeyFactory& ml_dsa_factory =
      ml_dsa_sign_key_manager_->get_key_factory();
  ...
  absl::StatusOr<std::unique_ptr<google::crypto::tink::KeyData>> key_data =
  ...
  EXPECT_THAT((*key_data)->key_material_type(),
              Eq(google::crypto::tink::KeyData::ASYMMETRIC_PRIVATE));
  ...
  EXPECT_THAT(private_key.key_value().size(), Eq(32));
  EXPECT_THAT(private_key.public_key().version(), Eq(0));
  EXPECT_THAT(private_key.public_key().key_value().size(), Eq(1952));
  EXPECT_THAT(private_key.public_key().version(), Eq(0));
  EXPECT_THAT(private_key.public_key().params().ml_dsa_instance(),
              Eq(google::crypto::tink::MlDsaInstance::ML_DSA_65));
}

TEST_F(LegacyKeyManagerImplTest, Factory_NewKey_WrongKeyFormat) {
  const KeyFactory& ml_dsa_factory =
      ml_dsa_sign_key_manager_->get_key_factory();

  auto key_format = google::crypto::tink::EcdsaKeyFormat();
  key_format.set_version(0);
  key_format.mutable_params()->set_hash_type(
      google::crypto::tink::HashType::SHA256);
  key_format.mutable_params()->set_curve(
      google::crypto::tink::EllipticCurveType::NIST_P256);
  key_format.mutable_params()->set_encoding(
...
TEST_F(LegacyKeyManagerImplTest, Factory_GetPublicKeyData) {
  ...
  const PrivateKeyFactory* ml_dsa_private_factory =
      dynamic_cast<const PrivateKeyFactory*>(&ml_dsa_factory);
  ASSERT_THAT(ml_dsa_private_factory, NotNull());

  absl::StatusOr<std::unique_ptr<google::crypto::tink::KeyData>>
      public_key_data =
          ml_dsa_private_factory->GetPublicKeyData((*key)->SerializeAsString());
  ASSERT_THAT(public_key_data, IsOk());

  EXPECT_THAT((*public_key_data)->type_url(),
              Eq("type.googleapis.com/google.crypto.tink.MlDsaPublicKey"));
  EXPECT_THAT((*public_key_data)->key_material_type(),
              Eq(google::crypto::tink::KeyData::ASYMMETRIC_PUBLIC));

  google::crypto::tink::MlDsaPublicKey public_key;
  public_key.ParseFromString((*public_key_data)->value());
  EXPECT_THAT(public_key.key_value().size(), Eq(1952));
  EXPECT_THAT(public_key.params().ml_dsa_instance(),
              Eq(google::crypto::tink::MlDsaInstance::ML_DSA_65));
}

TEST_F(LegacyKeyManagerImplTest, Factory_GetPublicKeyData_WrongKeyType) {
  const KeyFactory& ml_dsa_factory =
      ml_dsa_sign_key_manager_->get_key_factory();
  const PrivateKeyFactory* ml_dsa_private_factory =
      dynamic_cast<const PrivateKeyFactory*>(&ml_dsa_factory);
  ...
  google::crypto::tink::EcdsaPrivateKey ecdsa_key;
  ecdsa_key.set_version(0);
  ecdsa_key.set_key_value("whatever");
  ecdsa_key.mutable_public_key()->set_version(0);
  ecdsa_key.mutable_public_key()->set_x("foo");
  ecdsa_key.mutable_public_key()->set_y("bar");
  ecdsa_key.mutable_public_key()->mutable_params()->set_hash_type(
      google::crypto::tink::HashType::SHA256);
  ecdsa_key.mutable_public_key()->mutable_params()->set_curve(
      google::crypto::tink::EllipticCurveType::NIST_P256);
  ecdsa_key.mutable_public_key()->mutable_params()->set_encoding(
      google::crypto::tink::EcdsaSignatureEncoding::DER);

  absl::StatusOr<std::unique_ptr<google::crypto::tink::KeyData>>
      public_key_data = ml_dsa_private_factory->GetPublicKeyData(
          ecdsa_key.SerializeAsString());
  ASSERT_THAT(public_key_data,
              StatusIs(absl::StatusCode::kInvalidArgument,
...
constexpr absl::string_view kHexMessage65 =
    "D81C4D8D734FCBFBEADE3D3F8A039FAA2A2C9957E835AD55B22E75BF57BB556AC8";
constexpr absl::string_view kHexSignatureWithoutTinkPrefix65 =
    "BD0D51DB2F225AC6D3DA8F0C2439B0BCDA26EFF7EFA67CFD3C2B98EFA08477"
...
TEST_F(LegacyKeyManagerImplTest, GetPrimitive_KeyProto) {
  std::string public_key_bytes = HexDecodeOrDie(kHexPublicKey65);

  google::crypto::tink::MlDsaPublicKey public_key;
  public_key.set_version(0);
  public_key.set_key_value(public_key_bytes);
  public_key.mutable_params()->set_ml_dsa_instance(
      google::crypto::tink::MlDsaInstance::ML_DSA_65);

  absl::StatusOr<std::unique_ptr<PublicKeyVerify>> verifier =
      ml_dsa_verify_key_manager_->GetPrimitive(public_key);
  ASSERT_THAT(verifier, IsOk());

  std::string message_bytes = HexDecodeOrDie(kHexMessage65);
  std::string signature_bytes =
      HexDecodeOrDie(kHexSignatureWithoutTinkPrefix65);
  EXPECT_THAT((*verifier)->Verify(signature_bytes, message_bytes), IsOk());
}

TEST_F(LegacyKeyManagerImplTest, GetPrimitive_KeyData) {
  std::string public_key_bytes = HexDecodeOrDie(kHexPublicKey65);
  ...
  key_data.set_value(public_key.SerializeAsString());

  absl::StatusOr<std::unique_ptr<PublicKeyVerify>> verifier =
      ml_dsa_verify_key_manager_->GetPrimitive(key_data);
  ASSERT_THAT(verifier, IsOk());
