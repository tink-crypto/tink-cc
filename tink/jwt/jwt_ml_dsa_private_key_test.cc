...
const absl::string_view kOtherMlDsa44PrivateSeedBytes =
...
TEST(JwtMlDsaPrivateKeyTest, CreateWithInvalidPrivateSeedLength) {
  absl::StatusOr<JwtMlDsaParameters> parameters =
  ...
  RestrictedData long_private_seed = RestrictedData(
      test::HexDecodeOrDie(absl::StrCat("ff", kMlDsa44PrivateSeedBytes)),
      InsecureSecretKeyAccess::Get());

  RestrictedData short_private_seed =
      RestrictedData(test::HexDecodeOrDie(kMlDsa44PrivateSeedBytes.substr(
                         2, kMlDsa44PrivateSeedBytes.size())),
                     InsecureSecretKeyAccess::Get());
  ...
  EXPECT_THAT(
      ...
      StatusIs(
          ...
          HasSubstr(
              "Private key length 33 is different from expected length 32")));
  EXPECT_THAT(
      ...
      StatusIs(
          absl::StatusCode::kInvalidArgument,
          HasSubstr(
              "Private key length 31 is different from expected length 32")));
}
...
TEST(JwtMlDsaPrivateKeyTest, CreateMismatchedKeyPairFails) {
  ...
  RestrictedData other_private_seed =
      RestrictedData(test::HexDecodeOrDie(kOtherMlDsa44PrivateSeedBytes),
                     InsecureSecretKeyAccess::Get());
...
TEST(JwtMlDsaPrivateKeyTest, CopyAssignment) {
  ...
  RestrictedData other_private_seed =
      RestrictedData(test::HexDecodeOrDie(kMlDsa65PrivateSeedBytes),
                     InsecureSecretKeyAccess::Get());
