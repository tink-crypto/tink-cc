...
using JwtMlDsaParsePrefixTest =
    TestWithParam<google::crypto::tink::internal::OutputPrefixTypeTP>;

INSTANTIATE_TEST_SUITE_P(
    JwtMlDsaParsePrefixTestSuite, JwtMlDsaParsePrefixTest,
...
TEST_P(JwtMlDsaProtoSerializationTest, ParsePublicKeyWithMutableRegistry) {
  TestCase test_case = GetParam();
  MutableSerializationRegistry registry;
  ASSERT_THAT(RegisterJwtMlDsaProtoSerializationWithMutableRegistry(registry),
              IsOk());

  google::crypto::tink::JwtMlDsaPublicKey key_proto;
  key_proto.set_version(0);
  key_proto.set_algorithm(test_case.proto_algorithm);
  key_proto.set_key_value(test_case.public_key_bytes);
  ...
  absl::StatusOr<JwtMlDsaPublicKey> expected_key =
      builder.Build(GetPartialKeyAccess());
  ASSERT_THAT(expected_key, IsOk());
  EXPECT_THAT(**parsed_key, Eq(*expected_key));
}

TEST_P(JwtMlDsaProtoSerializationTest, ParsePublicKeyWithRegistryBuilder) {
  TestCase test_case = GetParam();
  SerializationRegistry::Builder builder;
  ASSERT_THAT(RegisterJwtMlDsaProtoSerializationWithRegistryBuilder(builder),
              IsOk());
...
TEST_F(JwtMlDsaProtoSerializationTest, ParseTinkPublicKeyWithCustomKidFails) {
  MutableSerializationRegistry registry;
  ASSERT_THAT(RegisterJwtMlDsaProtoSerializationWithMutableRegistry(registry),
              IsOk());
  ...
  google::crypto::tink::JwtMlDsaPublicKey key_proto;
  key_proto.set_version(0);
  key_proto.set_algorithm(JwtMlDsaAlgorithm::ML_DSA44);
  key_proto.set_key_value(test::HexDecodeOrDie(kMlDsa44PublicKeyBytes));
  ...
  // downstream while building JwtMlDsaPublicKey object.
...
TEST_F(JwtMlDsaProtoSerializationTest, ParsePublicKeyWithInvalidSerialization) {
  MutableSerializationRegistry registry;
  ASSERT_THAT(RegisterJwtMlDsaProtoSerializationWithMutableRegistry(registry),
              IsOk());
...
TEST_F(JwtMlDsaProtoSerializationTest, ParsePublicKeyWithInvalidVersion) {
  MutableSerializationRegistry registry;
  ASSERT_THAT(RegisterJwtMlDsaProtoSerializationWithMutableRegistry(registry),
              IsOk());
...
TEST_P(JwtMlDsaParsePrefixTest, ParsePublicKeyWithInvalidPrefix) {
  google::crypto::tink::internal::OutputPrefixTypeTP
      invalid_output_prefix_type = GetParam();
  MutableSerializationRegistry registry;
  ASSERT_THAT(RegisterJwtMlDsaProtoSerializationWithMutableRegistry(registry),
...
TEST_F(JwtMlDsaProtoSerializationTest, ParsePublicKeyWithUnknownAlgorithm) {
  MutableSerializationRegistry registry;
  ASSERT_THAT(RegisterJwtMlDsaProtoSerializationWithMutableRegistry(registry),
              IsOk());
  ...
  google::crypto::tink::JwtMlDsaPublicKey key_proto;
  key_proto.set_version(0);
  key_proto.set_algorithm(JwtMlDsaAlgorithm::ML_DSA_UNKNOWN);
  key_proto.set_key_value(test::HexDecodeOrDie(kMlDsa44PublicKeyBytes));
  RestrictedData serialized_key = RestrictedData(
      key_proto.SerializeAsString(), InsecureSecretKeyAccess::Get());
...
TEST_P(JwtMlDsaProtoSerializationTest, SerializePublicKeyWithMutableRegistry) {
  TestCase test_case = GetParam();
  MutableSerializationRegistry registry;
  ASSERT_THAT(RegisterJwtMlDsaProtoSerializationWithMutableRegistry(registry),
              IsOk());
  ...
  google::crypto::tink::JwtMlDsaPublicKey proto_key;
  ...
  EXPECT_THAT(proto_key.key_value(),
              Eq(key->GetPublicKeyBytes(GetPartialKeyAccess())));
...
TEST_P(JwtMlDsaProtoSerializationTest, SerializePublicKeyWithRegistryBuilder) {
  TestCase test_case = GetParam();
  SerializationRegistry::Builder builder;
  ASSERT_THAT(RegisterJwtMlDsaProtoSerializationWithRegistryBuilder(builder),
              IsOk());
