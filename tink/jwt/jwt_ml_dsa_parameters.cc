...
absl::StatusOr<JwtMlDsaParameters> JwtMlDsaParameters::Create(
  ...
  if (kSupportedKidStrategies->find(kid_strategy) ==
    ...
    return absl::Status(
        ...
        "Cannot create JWT ML-DSA parameters with unknown kid strategy.");
  }
  static const std::set<Algorithm>* kSupportedAlgorithms =
      new std::set<Algorithm>(
          {Algorithm::kMlDsa44, Algorithm::kMlDsa65, Algorithm::kMlDsa87});
  if (kSupportedAlgorithms->find(algorithm) == kSupportedAlgorithms->end()) {
    return absl::Status(
        absl::StatusCode::kInvalidArgument,
        "Cannot create JWT ML-DSA parameters with unknown algorithm.");
  }
  return JwtMlDsaParameters(kid_strategy, algorithm);
}

bool JwtMlDsaParameters::operator==(const Parameters& other) const {
  const JwtMlDsaParameters* that =
      dynamic_cast<const JwtMlDsaParameters*>(&other);
  if (that == nullptr) {
    return false;
  }
