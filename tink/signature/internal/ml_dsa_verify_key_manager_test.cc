...
class MlDsaVerifyKeyManagerTest : public ::testing::Test {
 protected:
  void SetUp() override {
    ASSERT_THAT(SignatureConfig::Register(), IsOk());
    key_manager_ = MakeMlDsaVerifyKeyManager();
  }

  std::unique_ptr<KeyManager<PublicKeyVerify>> key_manager_;
};

TEST_F(MlDsaVerifyKeyManagerTest, Basic) {
...
TEST_F(MlDsaVerifyKeyManagerTest, GetPrimitive) {
  auto public_key_manager = MakeMlDsaVerifyKeyManager();

  std::string public_key_bytes = HexDecodeOrDie(kHexPublicKey65);
  google::crypto::tink::MlDsaPublicKey ml_dsa_public_key;
  ml_dsa_public_key.set_version(0);
  ml_dsa_public_key.set_key_value(public_key_bytes);
  ml_dsa_public_key.mutable_params()->set_ml_dsa_instance(
      google::crypto::tink::ML_DSA_65);

  auto verifier = public_key_manager->GetPrimitive(ml_dsa_public_key);
