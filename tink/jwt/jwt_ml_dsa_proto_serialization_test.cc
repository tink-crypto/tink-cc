...
class JwtMlDsaProtoSerializationTest : public TestWithParam<TestCase> {
 protected:
  void SetUp() override {
...
INSTANTIATE_TEST_SUITE_P(
    JwtMlDsaProtoSerializationTestSuite, JwtMlDsaProtoSerializationTest,
    Values(
        TestCase{
...
TEST_P(JwtMlDsaProtoSerializationTest, ParseParameters) {
...
TEST_P(JwtMlDsaProtoSerializationTest, SerializeParameters) {
  TestCase test_case = GetParam();
  ASSERT_THAT(RegisterJwtMlDsaProtoSerialization(), IsOk());
...
TEST_P(JwtMlDsaProtoSerializationTest, ParsePublicKey) {
  TestCase test_case = GetParam();
  ASSERT_THAT(RegisterJwtMlDsaProtoSerialization(), IsOk());
...
TEST_P(JwtMlDsaProtoSerializationTest, SerializePublicKey) {
  TestCase test_case = GetParam();
  ASSERT_THAT(RegisterJwtMlDsaProtoSerialization(), IsOk());
  ...
  EXPECT_THAT(proto_key.key_value(), Eq(test_case.public_key_bytes));
...
TEST_P(JwtMlDsaProtoSerializationTest, ParsePrivateKey) {
  TestCase test_case = GetParam();
  ASSERT_THAT(RegisterJwtMlDsaProtoSerialization(), IsOk());
...
TEST_P(JwtMlDsaProtoSerializationTest, SerializePrivateKey) {
  TestCase test_case = GetParam();
  ASSERT_THAT(RegisterJwtMlDsaProtoSerialization(), IsOk());
