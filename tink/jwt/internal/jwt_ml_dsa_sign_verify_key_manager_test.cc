...
TEST(JwtMlDsaSignKeyManagerTest, GetPrimitive) {
  ...
  absl::StatusOr<std::unique_ptr<portable_proto::MessageLite>> private_key =
      key_manager->get_key_factory().NewKey(format);
  ASSERT_THAT(private_key, IsOk());

  absl::StatusOr<std::unique_ptr<crypto::tink::JwtPublicKeySignInternal>>
      signer = key_manager->GetPrimitive(**private_key);
  ...
  absl::StatusOr<std::string> compact =
      (*signer)->SignAndEncodeWithKid(*raw_jwt, /*kid=*/absl::nullopt);
  EXPECT_THAT(compact, IsOk());
  // TODO: b/508561531 - Add corresponding JWT signature verification.
}
