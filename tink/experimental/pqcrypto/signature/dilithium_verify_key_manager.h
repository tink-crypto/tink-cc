...
class DilithiumVerifyKeyManager
    : public KeyTypeManager<google::crypto::tink::DilithiumPublicKey, void,
 ...
 public:
  class PublicKeyVerifyFactory : public PrimitiveFactory<PublicKeyVerify> {
    absl::StatusOr<std::unique_ptr<PublicKeyVerify>> Create(
        const google::crypto::tink::DilithiumPublicKey& public_key)
        const override;
  };

  DilithiumVerifyKeyManager()
 ...
 private:
  const std::string key_type_ =
      absl::StrCat(kTypeGoogleapisCom,
                   google::crypto::tink::DilithiumPublicKey().GetTypeName());
};
