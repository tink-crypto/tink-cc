...
NewJwtMlDsaVerifyInternal(const JwtMlDsaPublicKey& jwt_ml_dsa_public_key) {
  ...
  absl::StatusOr<std::unique_ptr<PublicKeyVerify>> ml_dsa_verify_boringssl =
      internal::NewMlDsaVerifyBoringSsl(*ml_dsa_public_key);
  if (!ml_dsa_verify_boringssl.ok()) {
    return ml_dsa_verify_boringssl.status();
  }
  ...
  switch (jwt_ml_dsa_params.GetKidStrategy()) {
    case JwtMlDsaParameters::KidStrategy::kBase64EncodedKeyId: {
      std::string kid = jwt_ml_dsa_public_key.GetKid().value();
      return JwtPublicKeyVerifyImpl::WithKid(
          *std::move(ml_dsa_verify_boringssl), *algorithm_name, kid);
    }
    case JwtMlDsaParameters::KidStrategy::kCustom: {
      std::string custom_kid = jwt_ml_dsa_public_key.GetKid().value();
      return JwtPublicKeyVerifyImpl::RawWithCustomKid(
          *std::move(ml_dsa_verify_boringssl), *algorithm_name, custom_kid);
    }
    case JwtMlDsaParameters::KidStrategy::kIgnored:
      return JwtPublicKeyVerifyImpl::Raw(*std::move(ml_dsa_verify_boringssl),
                                         *algorithm_name);
    default:
      // Should never happen.
      return absl::Status(absl::StatusCode::kInternal,
