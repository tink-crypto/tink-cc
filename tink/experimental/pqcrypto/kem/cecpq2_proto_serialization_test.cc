...
TEST_F(Cecpq2ProtoSerializationTest, RegisterTwiceSucceeds) {
  ASSERT_THAT(RegisterCecpq2ProtoSerialization(), IsOk());
  ASSERT_THAT(RegisterCecpq2ProtoSerialization(), IsOk());
}
...
INSTANTIATE_TEST_SUITE_P(
    ...
    Values(TestCase{Cecpq2Parameters::Variant::kTink, /*salt=*/"salt",
                    internal::OutputPrefixTypeEnum::kTink, /*id=*/0x02030400,
...
TEST_P(Cecpq2ProtoSerializationTest, ParseParameters) {
...
TEST_P(Cecpq2ProtoSerializationTest, SerializeParameters) {
  TestCase test_case = GetParam();
  ASSERT_THAT(RegisterCecpq2ProtoSerialization(), IsOk());
...
TEST_P(Cecpq2ProtoSerializationTest, ParsePublicKey) {
  TestCase test_case = GetParam();
  ASSERT_THAT(RegisterCecpq2ProtoSerialization(), IsOk());
...
TEST_P(Cecpq2ProtoSerializationTest, SerializePublicKey) {
  TestCase test_case = GetParam();
  ASSERT_THAT(RegisterCecpq2ProtoSerialization(), IsOk());
  ...
  EXPECT_THAT(proto_serialization->GetKeyMaterialTypeEnum(),
              Eq(internal::KeyMaterialTypeEnum::kAsymmetricPublic));
...
TEST_P(Cecpq2ProtoSerializationTest, ParsePrivateKey) {
  TestCase test_case = GetParam();
  ASSERT_THAT(RegisterCecpq2ProtoSerialization(), IsOk());
...
TEST_P(Cecpq2ProtoSerializationTest, SerializePrivateKey) {
  TestCase test_case = GetParam();
  ASSERT_THAT(RegisterCecpq2ProtoSerialization(), IsOk());
  ...
  EXPECT_THAT(proto_serialization->GetKeyMaterialTypeEnum(),
              Eq(internal::KeyMaterialTypeEnum::kAsymmetricPrivate));
  EXPECT_THAT(proto_serialization->GetOutputPrefixTypeEnum(),
              Eq(test_case.output_prefix_type));
  EXPECT_THAT(proto_serialization->IdRequirement(),
              Eq(test_case.id_requirement));
