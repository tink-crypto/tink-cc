...
class JwtRsaSsaPkcs1Parameters : public JwtSignatureParameters {
 public:
  ...
  enum class KidStrategy : int {
    ...
    //
    // NOTE: Tink does not allow random generation of JWT RSASSA-PKCS1 key
  ...
  // Creates JWT RSASSA-PKCS1 parameters instances.
  class Builder {
   public:
    ...
    // Creates JWT RSASSA-PKCS1 parameters object from this builder. Fails if
    // required fields are not set. The public exponent defaults to F4,
    // if not specified.
    absl::StatusOr<JwtRsaSsaPkcs1Parameters> Build();

   private:
    static BigInteger CreateDefaultPublicExponent();

    std::optional<JwtRsaSsaPkcs1Parameters::Algorithm> algorithm_ =
        std::nullopt;
    std::optional<JwtRsaSsaPkcs1Parameters::KidStrategy> kid_strategy_ =
        std::nullopt;
    std::optional<int> modulus_size_in_bits_ = std::nullopt;
    // Defaults to F4.
    BigInteger public_exponent_ = CreateDefaultPublicExponent();
  };

  // Copyable and movable.
  JwtRsaSsaPkcs1Parameters(const JwtRsaSsaPkcs1Parameters& other) = default;
  JwtRsaSsaPkcs1Parameters& operator=(const JwtRsaSsaPkcs1Parameters& other) =
      default;
  JwtRsaSsaPkcs1Parameters(JwtRsaSsaPkcs1Parameters&& other) = default;
  JwtRsaSsaPkcs1Parameters& operator=(JwtRsaSsaPkcs1Parameters&& other) =
      default;
...
#endif  // TINK_JWT_JWT_RSA_SSA_PKCS1_PARAMETERS_H_
